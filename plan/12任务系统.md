...
建议联读
如果你刚从 s03 过来，先回 data-structures.md，重新确认 TodoItem / PlanState 和 TaskRecord 不是同一层状态。
如果你开始把“对象边界”读混，先回 entity-map.md，把 message、task、runtime task、teammate 这几层拆开。
如果你准备继续读 s13，建议把 s13a-runtime-task-model.md 先放在手边，因为从这里开始最容易把 durable task 和 runtime task 混成一个词。
先把几个词讲明白
什么是任务
这里的 task 指的是：
