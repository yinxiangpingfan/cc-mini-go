...
// NewTaskManager 创建 TaskManager，扫描 taskDir() 初始化 nextID。
...
// uniqueAppend 把 add 中不存在于 base 的元素追加到 base 返回新切片。
	...
	return &Tools{
		...
		Func: func(ctx context.Context, args map[string]any) string {
			...
			if !ok || strings.TrimSpace(subject) == "" {
				return jsonErr(fmt.Sprintf(errors.ErrToolFunctionCall, "subject"))
			}
...
func NewTaskUpdateTool(mgr *TaskManager) *Tools {
	return &Tools{
		...
		Func: func(ctx context.Context, args map[string]any) string {
			...
			if !ok {
				return jsonErr(fmt.Sprintf(errors.ErrToolFunctionCall, "task_id"))
			}
