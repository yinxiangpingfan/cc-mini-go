...
// editFile 对已存在 file 做精确字符串替换，返回替换次数。
...
func editFile(filePath string, oldString string, newString string, replaceAll bool) (int, error) {
	...
	recordedHash, ok := shared.ReadFileState.ReadFiles[filePath]
	...
	shared.ReadFileState.ReadFiles[filePath] = newHash
...
func NewEditFileTool() *Tools {
	return &Tools{
		...
		Func: func(ctx context.Context, args map[string]interface{}) string {
			...
			if !ok || oldString == "" {
				return shared.JsonErr(fmt.Sprintf(errors.ErrToolFunctionCall, "old_string"))
			}
			// new_string 允许为空（用于删除文本），仅要求类型正确
			newString, ok := args["new_string"].(string)
			if !ok {
				return shared.JsonErr(fmt.Sprintf(errors.ErrToolFunctionCall, "new_string"))
			}
			if oldString == newString {
				return shared.JsonErr(errors.ErrEditNoChange.Error())
			}
