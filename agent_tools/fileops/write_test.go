...
func resetReadFiles(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		for k := range shared.ReadFileState.ReadFiles {
			delete(shared.ReadFileState.ReadFiles, k)
		}
...
func TestWriteFile_NewFile(t *testing.T) {
	...
	_, ok := shared.ReadFileState.ReadFiles[f]
...
func TestWriteFile_ReadThenOverwrite(t *testing.T) {
	...
	shared.ReadFileState.MU.Lock()
	shared.ReadFileState.ReadFiles[f] = hash
	shared.ReadFileState.MU.Unlock()
