...
const (
	...
	PreviewChars = 2000
	// microCompactMinLen 短于该长度 of 工具结果不值得压缩
...
func NewSessionTranscript() *SessionTranscript {
	return &SessionTranscript{path: filepath.Join(shared.TranscriptDir(), "session.jsonl")}
}
...
func recentReadFiles() []string {
	...
	files := make([]string, 0, len(shared.ReadFileState.ReadFiles))
	for path := range shared.ReadFileState.ReadFiles {
