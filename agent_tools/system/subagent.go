...
func (r *SubAgentRunner) run(ctx context.Context, taskPrompt string) string {
	...
	childHandlers := make(map[string]shared.ToolFunc)
	...
	for turn := 0; turn < subAgentMaxTurns; turn++ {
		...
		if ctx.Err() != nil {
			return shared.JsonErr(ctx.Err().Error())
		}
		...
		if resp != nil && resp.StatusCode != 200 {
			...
			return shared.JsonErr(msg)
		}
		if err != nil {
			return shared.JsonErr(fmt.Errorf("%w: %w", errors.ErrSubAgentRequest, err).Error())
		}
...
func buildChildTools(handlers *map[string]shared.ToolFunc) []client.Tool {
	...
	todoList := planning.NewTodoListTool()
